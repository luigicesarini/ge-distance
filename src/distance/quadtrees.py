...
class Rect:
    ...
    def draw(self, ax, n_ul, timestamp, c='k', lw=1,**kwargs):
        x1, y1 = self.west_edge, self.north_edge
        x2, y2 = self.east_edge, self.south_edge
        pd.DataFrame([x1,y1,x2,y2]).transpose().\
            to_csv(f'/mnt/beegfs/lcesarini/ge-distance/res/coords_rect_{n_ul}_{timestamp}.csv', index=False, header=False, mode='a')
...
class QuadTree:
    ...
    def draw(self, ax, n_ul, timestamp):
        """Draw a representation of the quadtree on Matplotlib Axes ax."""

        self.boundary.draw(ax,n_ul=n_ul, timestamp=timestamp)
        if self.divided:
            # print(f'nw:{self.nw.boundary}')
            # print(f'ne:{self.ne.boundary}')
            self.nw.draw(ax,n_ul=n_ul,timestamp=timestamp)
            self.ne.draw(ax,n_ul=n_ul,timestamp=timestamp)
            self.se.draw(ax,n_ul=n_ul,timestamp=timestamp)
            self.sw.draw(ax,n_ul=n_ul,timestamp=timestamp)