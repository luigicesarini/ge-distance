...
class Rect:
    ...
    def draw(self, ax, n_ul, c='k', lw=1,**kwargs):
        x1, y1 = self.west_edge, self.north_edge
        x2, y2 = self.east_edge, self.south_edge
        pd.DataFrame([x1,y1,x2,y2]).transpose().\
            to_csv(f'/mnt/beegfs/lcesarini/ge-distance/res/test_{n_ul}.csv', index=False, header=False, mode='a')
        ax.plot([x1,x2,x2,x1,x1],[y1,y1,y2,y2,y1], c=c, lw=lw, **kwargs)
        ax.set_title(f'UL:{n_ul}')
...
class QuadTree:
    ...
    def divide(self):
        ...
        # print(f'nw:{self.nw.boundary}')
        # print(type(self.nw.boundary))
        self.divided = True
    ...
    def draw(self, ax, n_ul):
        """Draw a representation of the quadtree on Matplotlib Axes ax."""

        self.boundary.draw(ax,n_ul=n_ul)
        if self.divided:
            # print(f'nw:{self.nw.boundary}')
            # print(f'ne:{self.ne.boundary}')
            self.nw.draw(ax,n_ul=n_ul)
            self.ne.draw(ax,n_ul=n_ul)
            self.se.draw(ax,n_ul=n_ul)
            self.sw.draw(ax,n_ul=n_ul)
...
if __name__ == '__main__':
    ...
    lon=np.random.choice(np.linspace(domain.west_edge,domain.east_edge,1000),UL)
    lat=np.random.choice(np.linspace(domain.north_edge,domain.south_edge,1000),UL)
    ...
    ax = plt.subplot()
    qtree.draw(ax,n_ul=N_UL_SQUARE)
